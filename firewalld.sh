...
firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i $IF -s 127.0.0.1/8 -j DROP
firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i $IF -s 10.0.0.0/8 -j DROP #開発環境では許可
firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i $IF -s 172.16.0.0/12 -j DROP
# firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i $IF -s 192.168.0.0/16 -j DROP  #開発環境では許可
# firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 0 -i $IF -s 192.168.0.0/24  -j DROP #開発環境では許可
...
# add service
firewall-cmd --add-port=10000/tcp --zone=public --permanent # webmin
firewall-cmd --add-port=20222/tcp --zone=public --permanent # sshd
firewall-cmd --add-service=http --zone=public --permanent # httpd(80)
firewall-cmd --add-service=https --zone=public --permanent # httpd(443)

# ポートスキャン対策
firewall-cmd --permanent --direct --add-chain ipv4 filter port-scan
firewall-cmd --permanent --direct --add-rule ipv4 filter INPUT 400 -i $IF -p tcp --tcp-flags SYN,ACK,FIN,RST SYN -j port-scan
